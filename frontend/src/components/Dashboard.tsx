...
import {
  ...
  CirclePlus, ExternalLink, Filter, Gauge, Heart, Inbox, Layers3, LoaderCircle,
  LogOut, Menu, MessageCircleWarning, MessageSquareText, MoreHorizontal,
  Plus, RefreshCw, Search, Settings, Sparkles, ThumbsUp, Users, X,
} from 'lucide-react'
import {
        ...
        <nav className="main-nav">
          ...
          <div className="product-nav">
            {products.map((item, index) => (
              <button key={item.id} className={item.id === product?.id ? 'active' : ''} onClick={() => { setProduct(item); setMobileNav(false) }}>
        ...
        {!product ? (
          ...
          <div className="dashboard-content">
            <section className="page-heading">
              ...
              <div className="heading-actions"><button className="secondary" onClick={() => void loadProductData(product)}><RefreshCw size={17} className={dataLoading ? 'spin' : ''} /> Refresh</button><button className="primary" onClick={() => setShowIngest(true)}><Sparkles size={17} /> Collect feedback</button></div>
            </section>
...
function AnalyticsView({ analytics, comments, search, setSearch, complaintsOnly, setComplaintsOnly }: {
  ...
  return <>
    ...
    <section className="metric-grid">
      <MetricCard icon={<MessageSquareText />} label="Total conversations" value={compact(analytics.total_comments)} detail="Analyzed comments" tone="violet" />
      <MetricCard icon={<MessageCircleWarning />} label="Complaint rate" value={`${(analytics.complaint_rate * 100).toFixed(1)}%`} detail={`${analytics.complaint_count} complaints detected`} tone="coral" />
      ...
      <MetricCard icon={<Heart />} label="Average engagement" value={(analytics.average_likes || 0).toFixed(1)} detail="Likes per conversation" tone="amber" />
    </section>
    ...
    <section className="chart-grid">
      ...
      <div className="panel sentiment-panel">
        ...
        <div className="sentiment-content">
          <div className="donut-wrap"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={sentimentData} dataKey="value" innerRadius={58} outerRadius={78} paddingAngle={3} stroke="none" isAnimationActive={false}>{sentimentData.map((entry) => <Cell key={entry.name} fill={SENTIMENT_COLORS[entry.name]} />)}</Pie></PieChart></ResponsiveContainer><div className="donut-center"><strong>{analytics.total_comments ? Math.round((sentimentData[0].value / analytics.total_comments) * 100) : 0}%</strong><span>positive</span></div></div>
    ...
    <section className="chart-grid lower-grid">
      ...
      <div className="panel sources-panel">
        <PanelHeader title="Top source videos" subtitle="Where conversations are happening" />
        <div className="source-list">{analytics.videos.length ? analytics.videos.slice(0, 5).map((video, index) => <a key={video.video_id} href={`https://youtube.com/watch?v=${video.video_id}`} target="_blank" rel="noreferrer"><span className="source-rank">{String(index + 1).padStart(2, '0')}</span><div><strong>{video.title}</strong><span>{video.comment_count} comments · {formatScore(video.average_sentiment)} sentiment</span></div><ExternalLink size={15} /></a>) : <div className="empty-small"><Layers3 size={22} /><span>No sources collected yet</span></div>}</div>
      </div>
    </section>
    ...
    <section className="panel feedback-panel">
      ...
      <div className="comment-list">{comments.length ? comments.map((comment) => <article className="comment-row" key={comment.id}><div className={`sentiment-avatar ${comment.sentiment}`}>{comment.sentiment === 'positive' ? <ThumbsUp size={17} /> : comment.sentiment === 'negative' ? <AlertTriangle size={17} /> : <MessageSquareText size={17} />}</div><div className="comment-body"><div className="comment-meta"><strong>{comment.author}</strong><span>on {comment.video_title}</span><time>{relativeDate(comment.published_at)}</time></div><p>{comment.text}</p><div className="comment-tags">{comment.issue_categories.map((tag) => <span key={tag}>{titleCase(tag)}</span>)}{comment.is_complaint && <span className="complaint-tag">Complaint</span>}</div></div><div className="comment-likes"><ThumbsUp size={14} /> {comment.like_count}</div></article>) : <div className="empty-feedback"><Search size={28} /><strong>No conversations found</strong><span>Try a different search or collect more feedback.</span></div>}</div>
    </section>
  </>
}
...
function WorkspaceOnboarding({ onCreate, onSignOut }: { onCreate: (company: string, product: string, query: string) => Promise<void>; onSignOut: () => void }) {
  ...
  return <main className="onboarding-page"><div className="onboarding-top"><div className="auth-brand dark"><span className="brand-icon"><MessageSquareText size={20}/></span> Product Voice</div><button onClick={onSignOut}>Sign out</button></div><form className="onboarding-card" onSubmit={submit}><div className="step-track"><span className="active"/><span className={step === 2 ? 'active' : ''}/></div><div className="onboarding-icon">{step === 1 ? <Users size={25}/> : <Layers3 size={25}/>}</div><span className="section-kicker">Step {step} of 2</span><h1>{step === 1 ? 'Name your workspace' : 'Add your first product'}</h1><p>{step === 1 ? 'Usually your company or team name.' : 'Choose a product and the YouTube query we should monitor.'}</p>{step === 1 ? <label>Workspace name<input value={company} onChange={(e) => setCompany(e.target.value)} placeholder="Acme Product Team" autoFocus required/></label> : <><label>Product name<input value={product} onChange={(e) => setProduct(e.target.value)} placeholder="Acme Mobile" autoFocus required/></label><label>YouTube search query <span>Optional</span><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={`${product || 'Product'} review problems`}/></label></>}{error && <div className="form-error">{error}</div>}<button className="primary" disabled={loading}>{loading ? <LoaderCircle className="spin" size={19}/> : step === 1 ? 'Continue' : 'Create workspace'} {!loading && <ArrowUpRight size={18}/>}</button>{step === 2 && <button type="button" className="text-button" onClick={() => setStep(1)}>Back</button>}</form></main>
}
...
function ProductModal({ onClose, onSubmit }: { onClose: () => void; onSubmit: (name: string, query: string) => Promise<void> }) {
  ...
  return <Modal title="Add a product" subtitle="Start tracking customer conversations." onClose={onClose}><form className="modal-form" onSubmit={submit}><label>Product name<input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. iPhone 18" autoFocus required/></label><label>YouTube search query <span>Optional</span><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={`${name || 'Product'} review problems`}/></label>{error && <div className="form-error">{error}</div>}<div className="modal-actions"><button type="button" className="secondary" onClick={onClose}>Cancel</button><button className="primary" disabled={loading}>{loading ? <LoaderCircle className="spin" size={18}/> : <Plus size={18}/>} Add product</button></div></form></Modal>
}
...
function IngestModal({ product, token, onClose, onComplete }: { product: Product; token: string; onClose: () => void; onComplete: (message: string) => void }) {
  const [videos, setVideos] = useState(5); const [comments, setComments] = useState(100); const [replies, setReplies] = useState(false); const [loading, setLoading] = useState(false); const [error, setError] = useState('')
  async function submit(event: FormEvent) { event.preventDefault(); setLoading(true); setError(''); try { const result = await api.ingest(token, product.id, { max_videos: videos, max_comments_per_video: comments, include_replies: replies }); onComplete(`${result.comments_indexed} comments collected from ${result.videos_processed} videos`) } catch (caught) { setError(caught instanceof Error ? caught.message : 'Ingestion failed'); setLoading(false) } }
  return <Modal title={`Collect feedback for ${product.name}`} subtitle={`Searching YouTube for “${product.youtube_query}”`} onClose={onClose}><form className="modal-form" onSubmit={submit}><div className="field-row"><label>Videos<input type="number" min="1" max="50" value={videos} onChange={(e) => setVideos(Number(e.target.value))}/></label><label>Comments per video<input type="number" min="1" max="1000" value={comments} onChange={(e) => setComments(Number(e.target.value))}/></label></div><label className="check-row"><input type="checkbox" checked={replies} onChange={(e) => setReplies(e.target.checked)}/><span><strong>Include replies</strong><small>More context, but uses additional API quota.</small></span></label><div className="quota-note"><Gauge size={17}/><span>Up to <strong>{compact(videos * comments)}</strong> comments will be processed.</span></div>{error && <div className="form-error">{error}</div>}<div className="modal-actions"><button type="button" className="secondary" onClick={onClose} disabled={loading}>Cancel</button><button className="primary" disabled={loading}>{loading ? <><LoaderCircle className="spin" size={18}/> Collecting…</> : <><Sparkles size={18}/> Start collection</>}</button></div></form></Modal>
}
